...
/* Posts { sku, quantity } items to the RPi print server's /dry-print route.
    That route only cares about sku/quantity -- not which shipment they came
    from -- so callers pass the items array straight through, not the
...
async function submitPrintJob(items) {
    let response;
    try {
        response = await fetch(`${PRINT_SERVER_URL}/dry-print`, {
            method: 'POST',
            headers: {
